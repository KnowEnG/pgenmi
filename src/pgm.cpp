...
long double pgm::alpha_mstep(const table& dt,
                             const vector<vector<long double> >& post) {
  long double numer = 0;
  long double denom = 0;
  for (int i = 0; i < dt.num_rows; i++) {
    numer += post[i][1];
...
long double pgm::loglikelihood(const table& dt, const model& h) {
  ...
  for (int i = 0; i < dt.num_rows; i++) {
    loglik +=
        log(sig(dt.data[i], h.w, mask) * (dbeta(dt.pval[i], h.alpha) - 1) + 1);
  }
  return loglik;
}

// Computes loglikelihood of data in dt using _w and _alpha
long double pgm::loglikelihood(const table& dt, const vector<long double>& w,
...
void pgm::train(const table& dt, model& h) {
  ...
  while ((num_loglik_decr < max_loglik_decr || h.iter < min_iter) &&
         h.iter < max_iter) {
...
string pgm::header() {
  ...
  ss << "TRAINED_ITER"
     ...
     << "TRAINED_LOGLIK"
     << "\t";
  ss << "TRAINED_LOGLIK_PREV"
     << "\t" << vec2str(w_names, "TRAINED_") << "\t";
  ss << "TRAINED_ALPHA";
  return ss.str();
}
