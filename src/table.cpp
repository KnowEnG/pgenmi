...
table::table(const string& _filename)
    : filename(_filename),
      num_non_zero(),
      num_features(0),
      num_rows(0),
...
long double table::_cast_string_to_long_double(const vector<string>& elem,
                                               int index, int num_rows) {
  try {
    return stold(elem[index]);
  } catch (const out_of_range& err) {
    cerr << "Out of range error: " << err.what() << "\n";
    cerr << "Trying to cast element (" << num_rows << ", " << index
         << ") = " << elem[index] << "  to long double\n";
    exit(1);
  }
  return -1;
}
...
int table::readfile(const string& filename) {
  ...
  while (getline(ifile, line)) {
    ...
    // Check if it has the correct number of columns (num_features + gene +
    // pval)
    if (elem.size() != (num_features + 2)) {
      cerr << "Error: Row " << num_rows + 1 << " has " << elem.size() << " not "
    ...
    // Get row_names for first element and push_back a vector of (num_features)
    // zeros.
    ...
    for (int i = 0; i < num_features; i++) {
      // Get value
      data[num_rows - 1][i] =
          _cast_string_to_long_double(elem, i + 2, num_rows - 1);
